...
from black_engine.championship_policy import LONG_HORIZON_RESOURCE_CARDS
...
def audit_episode(path: str | Path, agent_name: str) -> EpisodeAudit:
    ...
    for step_index, pair in enumerate(steps[:-1]):
        ...
        if minimum > 0 and not options:
            ...
            audit.findings.append(_finding(step=step_index, turn=turn, seat=seat, code="MANDATORY_EMPTY", severity="FATAL", recorded=action if isinstance(action, list) else [], expected=None, runner_id=None, evidence={"context": obs["select"].get("context")}))
        ...
        if not _legal(obs, recorded):
            audit.findings.append(_finding(step=step_index, turn=turn, seat=seat, code="ILLEGAL_RECORDED_ACTION", severity="FATAL", recorded=recorded, expected=None, runner_id=None, evidence={"min": obs["select"].get("minCount"), "max": obs["select"].get("maxCount"), "options": len(options)}))
        ...
        if terminal is not None and actual != terminal:
            audit.findings.append(_finding(step=step_index, turn=turn, seat=seat, code="TERMINAL_ACTION_MISS", severity="FATAL", recorded=recorded, expected=[terminal], runner_id="TERMINAL_ACTION_FREEZE", evidence={"opponent_hp": context["opponent_hp"], "our_prizes": truth.our_prizes}))
            counts["TERMINAL_ACTION_MISS"] += 1
        elif lethal is not None and actual != lethal:
            audit.findings.append(_finding(step=step_index, turn=turn, seat=seat, code="LETHAL_ACTION_MISS", severity="MAJOR", recorded=recorded, expected=[lethal], runner_id="IMMEDIATE_KO_CHECK", evidence={"opponent_hp": context["opponent_hp"], "our_prizes": truth.our_prizes}))
            ...
            audit.findings.append(_finding(step=step_index, turn=turn, seat=seat, code=code, severity=severity, recorded=recorded, expected=[promotion], runner_id=policy.last_runner_id, evidence={"observed_opponent_damage": context["observed_opponent_damage"]}))
            counts[code] += 1
        if actual is not None and 0 <= actual < len(truth.options):
            ...
            if chosen.action_type == T_ENERGY and plan.plan.plan_id in BAD_ENERGY_PLAN_IDS:
                ...
                if best_index is not None and best_index != actual and best_plan is not None:
                    audit.findings.append(_finding(step=step_index, turn=turn, seat=seat, code="ENERGY_ATTACH_SUBOPTIMAL", severity="MAJOR", recorded=recorded, expected=[best_index], runner_id=best_plan.plan.plan_id, evidence={"chosen_plan": plan.plan.plan_id, "best_plan": best_plan.plan.plan_id, "target_serial": chosen.target_serial, "energy_card_id": chosen.card_id}))
                    counts["ENERGY_ATTACH_SUBOPTIMAL"] += 1
            if plan.plan.plan_id == "REJECT_NONPERSISTENT_DAMAGE":
                audit.findings.append(_finding(step=step_index, turn=turn, seat=seat, code="NONPERSISTENT_DAMAGE_REPEAT", severity="MAJOR", recorded=recorded, expected=None, runner_id=plan.plan.plan_id))
                counts["NONPERSISTENT_DAMAGE_REPEAT"] += 1
            if plan.plan.plan_id == "ATTACK_WITHOUT_BACKUP":
                audit.findings.append(_finding(step=step_index, turn=turn, seat=seat, code="ATTACK_WITHOUT_BACKUP", severity="MAJOR", recorded=recorded, expected=None, runner_id=plan.plan.plan_id))
            ...
            if context["deck_clock_critical"] and chosen.action_type == T_PLAY and chosen.card_id in LONG_HORIZON_RESOURCE_CARDS:
                audit.findings.append(_finding(step=step_index, turn=turn, seat=seat, code="DECK_CLOCK_VIOLATION", severity="MAJOR", recorded=recorded, expected=None, runner_id="DECK_CLOCK_SUPPRESS_RESOURCE", evidence={"deck_count": context["deck_count"], "safe_draw_budget": context["safe_draw_budget"]}))
    ...
    domains = {"runtime": 100.0, "terminal": 100.0, "promotion": 100.0, "energy": 100.0, "spread": 100.0, "attack": 100.0, "clock": 100.0, "tempo": 100.0}
    total_penalty = 0.0
