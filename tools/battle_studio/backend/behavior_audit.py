...
def _event_types(frame: Mapping[str, Any], player: int) -> Counter[str]:
    return Counter(str(event.get("type", "")).lower() for event in _seq(frame.get("events")) if isinstance(event, Mapping) and event.get("actor") in (player, None))


def _attack_available(frame: Mapping[str, Any]) -> bool:
    decision = _mapping(frame.get("decision"))
    return any(isinstance(item, Mapping) and str(item.get("kind", "")).lower() == "attack" for item in _seq(decision.get("candidates")))
...
def audit_frames(frames: Iterable[Mapping[str, Any]], subject_player: int = 0) -> dict[str, Any]:
    ...
    for index in range(1, len(items)):
        ...
        before_players = _seq(before.get("players")); after_players = _seq(after.get("players"))
        if len(before_players) <= subject_player or len(after_players) <= subject_player:
            continue
        bp = _mapping(before_players[subject_player]); ap = _mapping(after_players[subject_player])
        b_active = _mapping(bp.get("active")); a_active = _mapping(ap.get("active"))
        ...
        if len(switches) >= 2:
            i1, from1, to1 = switches[-2]; i2, from2, to2 = switches[-1]
    ...
    return {"schemaVersion": "BLACK_BEHAVIOR_AUDIT_V1", "frames": len(items), "subjectPlayer": subject_player, "findingCount": len(findings), "categoryCounts": dict(category_counts), "severityCounts": dict(severity_counts), "findings": [item.as_dict() for item in findings], "gate": "PASS" if not severity_counts.get("high") else "HOLD"}
