...
def _safe_text(value: Any, fallback: str = "", limit: int = 240) -> str:
    text = " ".join(
        str(value if value is not None else fallback)
        .replace("\r", " ")
        .replace("\n", " ")
        .split()
    )
    ...
    if (
        ...
        or _HEX_RE.search(text)
        or _JSONISH_RE.search(text)
    ):
...
def _safe_url(value: Any) -> str:
    ...
    allowed = any(host == item or host.endswith("." + item) for item in _ALLOWED_URL_HOSTS)
...
class PublicBattleView:
    ...
    def _card(self, raw: Any, player: int) -> dict[str, Any] | None:
        ...
        return {
            ...
            if isinstance(raw.get("slot"), int) and raw.get("slot") >= 0
            else None,
            "hp": _integer(raw.get("hp"))
            if isinstance(raw.get("hp"), int) and raw.get("hp") >= 0
            else None,
            "maxHp": _integer(raw.get("maxHp"))
            if isinstance(raw.get("maxHp"), int) and raw.get("maxHp") >= 0
            else None,
            "damage": max(0, _integer(raw.get("damage"))),
            "energies": [
                self._attachment_label(item, "エネルギー")
                for item in _seq(raw.get("energies"))
            ][:16],
            "tools": [
                self._attachment_label(item, "ポケモンのどうぐ")
                for item in _seq(raw.get("tools"))
            ][:8],
            "status": [
                _safe_text(item, "状態異常", 48)
                for item in _seq(raw.get("status"))
            ][:8],
            "evolution": [
    ...
    def _player(self, raw: Any, player: int) -> dict[str, Any]:
        ...
        hand = (
            [
                card
                for item in _seq(value.get("hand"))
        ...
        return {
            ...
            "bench": [
                card
                for item in _seq(value.get("bench"))
                if (card := self._card(item, player)) is not None
            ][:5],
            "hand": hand,
            ...
            "discard": [
                card
                for item in _seq(value.get("discard"))
                if (card := self._card(item, player)) is not None
            ][:60],
    ...
    def _event(self, raw: Any) -> dict[str, Any] | None:
        ...
        actor = raw.get("actor")
        text = (
            "対戦AIがこの行動を完了できませんでした"
            if event_type == "agent_error"
            else _safe_text(raw.get("text"), "対戦状態が更新されました")
        )
        return {
    ...
    def _candidate(self, raw: Any) -> dict[str, Any] | None:
        ...
        return {
            ...
            "reason": _safe_text(raw.get("reason"), "理由未提供")
    ...
    def _search_tree(self, raw: Any, path: str = "root") -> dict[str, Any] | None:
        ...
        return {
            ...
            "visits": max(0, _integer(raw.get("visits")))
            if raw.get("visits") is not None
            else None,
    ...
    def _decision(self, raw: Any) -> dict[str, Any] | None:
        ...
        candidates = [
            item
            for value in _seq(raw.get("candidates"))
            if (item := self._candidate(value)) is not None
        ][:32]
        alternatives = [
            item
            for value in _seq(raw.get("alternatives"))
            if (item := self._candidate(value)) is not None
        ][:32]
        raw_chosen = str(raw.get("chosen", ""))
        if not raw_chosen or re.fullmatch(r"\s*\[[0-9,\s-]*\]\s*", raw_chosen):
            chosen = next(
                (item["label"] for item in candidates if item["selected"]),
                "行動を選択",
            )
        else:
        ...
        decision: dict[str, Any] = {
            ...
            "warnings": [
                _safe_text(item, "注意事項", 160)
                for item in _seq(raw.get("warnings"))
            ][:16],
            "alternatives": alternatives,
            "boardDiff": [
                _safe_text(item, "盤面が変化しました", 160)
                for item in _seq(raw.get("boardDiff"))
            ][:16],
        }
            ...
            rejected.append(
                {
                    ...
                    "evidence": [
                        _safe_text(item, "公開可能な証拠なし", 160)
                        for item in _seq(value.get("evidence"))
                    ][:16],
                    "metrics": metrics,
                    "killedBy": [
                        _safe_text(item, "安全条件", 120)
                        for item in _seq(value.get("killedBy"))
                    ][:16],
                }
            )
        if rejected:
    ...
    def render(self, frame: Mapping[str, Any]) -> tuple[dict[str, Any], list[dict[str, Any]]]:
        ...
        public_frame = {
            ...
            "actingPlayer": _integer(frame.get("actingPlayer"))
            if frame.get("actingPlayer") in (0, 1)
            else 0,
            "phase": phase if phase in _PUBLIC_PHASES else "unknown",
            "players": [
                self._player(players[0], 0),
                self._player(players[1], 1),
            ],
            "stadium": self._card(frame.get("stadium"), 0),
            "events": [
                event
                for item in _seq(frame.get("events"))[-100:]
                if (event := self._event(item)) is not None
            ],
            "decision": self._decision(frame.get("decision")),
            "result": _safe_text(frame.get("result"), "対戦終了", 160)
