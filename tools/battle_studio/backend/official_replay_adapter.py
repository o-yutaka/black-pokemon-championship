...
def _card(raw: Mapping[str, Any], player: int, zone: str, slot: int | None) -> dict[str, Any]:
    ...
    energies = []
    for item in _seq(raw.get("energyCards", raw.get("energies", []))):
        energies.append(str(item.get("name", item.get("id", "?"))) if isinstance(item, Mapping) else str(item))
    tools = []
    for item in _seq(raw.get("tools", [])):
        tools.append(str(item.get("name", item.get("id", "?"))) if isinstance(item, Mapping) else str(item))
    evolution = []
    for item in _seq(raw.get("preEvolution", raw.get("evolution", []))):
        evolution.append(_int(item.get("id", item.get("cardId"))) if isinstance(item, Mapping) else _int(item))
    status = [str(item) for item in _seq(raw.get("status", raw.get("statuses", [])))]
    return {
        "playerIndex": player, "serial": serial, "cardId": card_id, "name": str(raw.get("name", f"Card #{card_id}")),
        "zone": zone, "slot": slot, "hp": hp_int, "maxHp": max_hp_int, "damage": max(0, damage),
        "energies": energies, "tools": tools, "status": status, "evolution": evolution, "imageUrl": None,
    }
...
def _player(raw: Mapping[str, Any], player: int) -> dict[str, Any]:
    ...
    return {
        "name": str(raw.get("name", raw.get("playerName", f"Player {player + 1}"))),
        "active": active_items[0] if active_items else None,
        "bench": bench, "hand": hand,
        "handCount": max(_int(raw.get("handCount"), len(hand)), len(hand)),
        "deckCount": max(0, _int(raw.get("deckCount"), len(_seq(raw.get("deck", []))))),
        "prizeCount": max(0, _int(raw.get("prizeCount"), len(_seq(raw.get("prize", raw.get("prizes", [])))))),
        "discard": discard, "supporterPlayed": bool(raw.get("supporterPlayed", False)),
        "retreated": bool(raw.get("retreated", raw.get("retreatUsed", False))),
    }
...
def normalize_official_frame(raw: Mapping[str, Any], frame_id: int) -> dict[str, Any]:
    ...
    return {
        "frameId": frame_id, "turn": max(0, _int(raw.get("turn", state.get("turn", 0)))),
        "actionCount": max(0, _int(raw.get("actionCount", state.get("turnActionCount", frame_id)))),
        "actingPlayer": acting, "phase": "result" if _result(state.get("result")) else str(raw.get("phase", "main")),
        "players": [_player(players_raw[0], 0), _player(players_raw[1], 1)], "stadium": None,
        "events": events, "decision": normalized_decision, "result": _result(state.get("result", raw.get("result"))),
    }
