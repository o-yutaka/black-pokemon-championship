...
_ACTION_KIND = {7: "trainer", 8: "ability", 9: "evolve", 10: "switch", 12: "retreat", 13: "attack", 14: "pass"}

def _selected_positions(action: Any) -> set[int]:
    return {item for item in action if type(item) is int and item >= 0} if isinstance(action, list) else set()
...
def _cabt_frames(value: Mapping[str, Any], subject_player: int) -> list[Mapping[str, Any]]:
    ...
    for step_index, step in enumerate(steps):
        if not isinstance(step, list) or len(step) <= subject_player: continue
        record = step[subject_player]
        if not isinstance(record, Mapping): continue
        observation = record.get("observation")
        if not isinstance(observation, Mapping): continue
        current = observation.get("current")
        if not isinstance(current, Mapping): continue
        select = observation.get("select") if isinstance(observation.get("select"), Mapping) else {}
        ...
        for position, option in enumerate(options if isinstance(options, list) else []):
            if not isinstance(option, Mapping): continue
            raw_type = option.get("type")
            kind = _ACTION_KIND.get(raw_type, "other") if isinstance(raw_type, int) else str(raw_type or "other").lower()
            candidates.append({"kind": kind, "selected": position in selected, "position": position, "option": dict(option)})
        frame = dict(current); frame["stepIndex"] = step_index; frame["players"] = current.get("players", [])
        frame["events"] = current.get("logs", observation.get("logs", []))
        frame["decision"] = {"candidates": candidates, "selectedPositions": sorted(selected), "context": select.get("context")}
        frames.append(frame)
    return frames
...
def _frames(value: Any, subject_player: int) -> list[Mapping[str, Any]]:
    ...
    for key in ("frames", "replay", "snapshots"):
        ...
        if isinstance(nested, list):
            if any(not isinstance(item, Mapping) for item in nested): raise ValueError(f"壊れた明示的{key}配列です")
            return list(nested)
    for key in ("output", "result", "payload", "data"):
        found = _frames(value.get(key), subject_player)
        if found: return found
    if isinstance(value.get("frame"), Mapping): return [value["frame"]]
    return []

def load_frames(path: Path, subject_player: int = 0) -> list[Mapping[str, Any]]:
    text = path.read_text(encoding="utf-8-sig")
    try:
        frames = _frames(json.loads(text), subject_player)
    ...
    if not frames: raise ValueError("監査可能なframesが見つかりません")
    return frames

def main() -> int:
    parser = argparse.ArgumentParser(); parser.add_argument("replay", type=Path); parser.add_argument("--subject-player", type=int, choices=(0,1), default=0); parser.add_argument("--out", type=Path); args = parser.parse_args()
    ...
    print(encoded, end=""); return 0 if result["gate"] == "PASS" else 2
