...
function DecisionInspector({ frame }: { frame: BattleFrame }) {
  ...
  return <aside className="inspector"><h2>Decision</h2>
    ...
    <h2>Events</h2><div className="event-log">{frame.events.length ? frame.events.map((event, index) => <div key={`${event.type}-${index}`}><span>{event.type}</span><p>{event.text}</p></div>) : <p className="muted">No event attached.</p>}</div>
  </aside>;
}
...
export default function App() {
  ...
  const loadFile = async (file: File | undefined) => {
    ...
    catch (caught) { setError(caught instanceof Error ? caught.message : "Replay load failed"); }
  };
  ...
  return <main className="app-shell">
    ...
    <section className="controls" aria-label="Replay controls"><div className="control-buttons"><button type="button" onClick={() => setFrameIndex(0)} disabled={frameIndex === 0}>⏮</button><button type="button" onClick={() => setFrameIndex((value) => Math.max(0, value - 1))} disabled={frameIndex === 0}>◀</button><button className="primary" type="button" onClick={() => setPlaying((value) => !value)}>{playing ? "Pause" : "Play"}</button><button type="button" onClick={() => setFrameIndex((value) => Math.min(replay.frames.length - 1, value + 1))} disabled={frameIndex === replay.frames.length - 1}>▶</button><button type="button" onClick={() => setFrameIndex(replay.frames.length - 1)} disabled={frameIndex === replay.frames.length - 1}>⏭</button></div><label className="timeline-label">Frame {Math.min(frameIndex + 1, replay.frames.length)}/{replay.frames.length}<input type="range" min="0" max={Math.max(0, replay.frames.length - 1)} value={Math.min(frameIndex, replay.frames.length - 1)} onChange={(event) => { setPlaying(false); setFrameIndex(Number(event.target.value)); }} style={{ "--progress": `${progress}%` } as React.CSSProperties} /></label><label className="speed-label">Speed<select value={speed} onChange={(event) => setSpeed(Number(event.target.value) as (typeof SPEEDS)[number])}>{SPEEDS.map((value) => <option key={value} value={value}>{value}×</option>)}</select></label></section>
